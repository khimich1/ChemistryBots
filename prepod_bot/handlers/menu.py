...
async def _rebuild_day_menu(message: types.Message, msg_id: int, day: str) -> None:
    ...
    rows.append([types.InlineKeyboardButton(text="⬅️ Назад", callback_data=f"calendar_back:{monday.strftime('%Y-%m-%d')}")])
...
async def on_pick_student(cb: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    time_str = data.get("lesson_time")
    # Если время уже выбрано ранее — сразу создаём запись(и)
    if time_str:
        try:
            start_s, end_s = [t.strip() for t in time_str.split("-", 1)]
        except Exception:
            # Если формат неожиданно сломан — попросим время снова
            await cb.message.answer(f"Введите время для {day} в формате HH:MM-HH:MM (например, 18:00-19:00):")
            await state.set_state(LessonStates.waiting_time)
            return
        # Попробуем найти username/label
        ...
        try:
            ...
            pass
        # Поддержка многонедельных добавлений
        days8 = data.get("lesson_days")
        if days8:
            for d in list(days8):
                add_lesson(cb.from_user.id, day=d, start=start_s, end=end_s, kind="student", target=target)
            first_day = days8[0]
            await _refresh_week(cb.message, state, first_day)
            until = bool(data.get("lesson_until_may30"))
            await cb.message.answer("✅ Уроки до 30 мая добавлены." if until else "✅ Уроки на 8 недель добавлены.")
        else:
            add_lesson(cb.from_user.id, day=day, start=start_s, end=end_s, kind="student", target=target)
            await _refresh_week(cb.message, state, day)
            await cb.message.answer("✅ Урок добавлен.")
        await state.set_state(None)
        return
    # Иначе просим время
    await cb.message.answer(f"Введите время для {day} в формате HH:MM-HH:MM (например, 18:00-19:00):")
...
@router.callback_query(lambda c: c.data and c.data.startswith("pick_lesson_group:"))
async def on_pick_group(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    try:
    ...
    try:
        ...
        await cb.message.answer("Ошибка времени. Начните заново: нажмите кнопку добавления ещё раз.")
        await state.set_state(None)
        return
    ...
    if days8:
        ...
        add_lesson(cb.from_user.id, day=day, start=start, end=end, kind="group", target=str(group_no))
        await _refresh_week(cb.message, state, day)
        await cb.message.answer("✅ Урок добавлен.")
    await state.set_state(None)
...
async def pick_broadcast_group(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cb.answer("Не понимаю номер группы", show_alert=True)
        return
    await state.update_data(broadcast_group=group_no)
...
async def on_open_lesson(cb: types.CallbackQuery, state: FSMContext):
    ...
    rows = [
        ...
        [types.InlineKeyboardButton(text="⬅️ Назад", callback_data="calendar_back:auto")],
    ]
...
@router.callback_query(lambda c: c.data and c.data.startswith("calendar_back:"))
async def calendar_back(cb: types.CallbackQuery, state: FSMContext):
    """Единое поведение для кнопки "Назад" в календаре:
    - Удаляет текущее сообщение и все временные
    - Удаляет верхний экран дня (если есть)
    - Открывает недельный календарь для указанной даты (или текущей недели при auto)
    Формат callback_data: calendar_back:YYYY-MM-DD | calendar_back:auto
    """
    await cb.answer()
    # 1) Удаляем сообщение, по которому кликнули
    with contextlib.suppress(Exception):
        await cb.message.delete()
    # 2) Чистим временные сообщения
    try:
        await _clear_temp_msgs(cb.message, state)
    except Exception:
        pass
    # 3) Удаляем верхний экран дня
    try:
        data0 = await state.get_data()
        ...
        pass
    # 4) Определяем, какую неделю показать
    from datetime import datetime as _datetime, date as _date
    try:
        arg = (cb.data.split(":", 1)[1] or "").strip()
    except Exception:
        arg = "auto"
    base_date = None
    if arg and arg != "auto":
        with contextlib.suppress(Exception):
            base_date = _datetime.strptime(arg, "%Y-%m-%d").date()
    # 5) Показываем недельный календарь
    await _show_week_schedule(cb.message, state, base_date=base_date, tg_id=cb.from_user.id)
...
async def on_add_menu(cb: types.CallbackQuery, state: FSMContext):
    await cb.answer()
    day = cb.data.split(":", 1)[1]
    # Как раньше: сначала показываем выбор, что добавить
    await state.update_data(lesson_day=day, lesson_kind=None, lesson_student_id=None, lesson_group_no=None)
    rows = [
        ...
        [types.InlineKeyboardButton(text="⬅️ Назад", callback_data=f"calendar_back:{day}")],
    ]
...
async def wg_add_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            ...
            reply_markup=get_group_pick_keyboard(groups or [], prefix="pick_add_group", include_create=True)
        )
    except Exception:
    ...
    # Ждём выбора кнопкой; ввод числа используем только если пользователь напишет вручную
...
@router.callback_query(lambda c: c.data == "wg_create_group")
async def wg_create_group(cb: types.CallbackQuery, state: FSMContext):
    """Начинает создание новой группы: просим номер/название, затем username."""
    await cb.answer()
    try:
        await cb.message.edit_text("Введите номер новой группы (целое число):")
    except Exception:
        await cb.message.answer("Введите номер новой группы (целое число):")
    await state.set_state(WorkGroups.waiting_group_number)
...
async def pick_add_group(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await cb.answer("Не понимаю номер", show_alert=True)
        return
    await state.update_data(group_no=group_no)
    ...
    try:
        await cb.message.edit_text(
            f"Участники группы {group_no}:",
            reply_markup=get_group_members_keyboard(members, group_no=group_no, back_cb="manage_groups_list")
        )
    except Exception:
        await cb.message.answer(
            f"Участники группы {group_no}:",
    ...
    # Ожидание username начнётся после нажатия «➕ Добавить в группу»
    await state.set_state(None)
...
@router.callback_query(lambda c: c.data.startswith("wg_prompt_add:"))
async def wg_prompt_add(cb: types.CallbackQuery, state: FSMContext):
    """Запрашивает username для добавления в выбранную группу."""
    await cb.answer()
    try:
        group_no = int(cb.data.split(":", 1)[1])
    except Exception:
    ...
    try:
        await cb.message.edit_text(
            f"Группа {group_no}. Теперь отправьте username ученика с @ (например, @ivan_ivanov):"
        )
    except Exception:
        await cb.message.answer(
            f"Группа {group_no}. Теперь отправьте username ученика с @ (например, @ivan_ivanov):"
...
async def wg_tasks_group(message: types.Message, state: FSMContext):
    ...
    if tasks:
        try:
            ...
            await message.answer("Выберите набор:", reply_markup=get_group_tasks_kb(tasks, group_no))
    else:
        # Даже если наборов нет — показываем кнопку создания
        await message.answer("У этой группы пока нет наборов:", reply_markup=get_group_tasks_kb([], group_no))
    # Больше не спрашиваем название сразу. Пользователь нажмёт «Создать задание».
    await state.set_state(None)


@router.callback_query(lambda c: c.data.startswith("wg_task_create:"))
async def wg_task_create(cb: types.CallbackQuery, state: FSMContext):
    """Начинает создание нового набора: запрашиваем название после нажатия кнопки."""
    await cb.answer()
    try:
        group_no = int(cb.data.split(":", 1)[1])
    except Exception:
...
async def pick_tasks_group(cb: types.CallbackQuery, state: FSMContext):
    ...
    if tasks:
        ...
        try:
            await _safe_edit_text(cb.message, "У этой группы пока нет наборов:", reply_markup=get_group_tasks_kb([], group_no))
        except Exception:
            await cb.message.answer("У этой группы пока нет наборов:", reply_markup=get_group_tasks_kb([], group_no))
    # Ждём нажатия «Создать задание»
    await state.set_state(None)
